...
CMAKE_FILES_TO_FORMAT=$(shell find . -type f -name CMakeLists.txt -o -name '*.cmake' | grep -v './cmake-build' | grep -v './libs' | sort)
CMAKE_FILES_TO_FORMAT+='libs/CMakeLists.txt'

TEST_FILES_TO_FORMAT=$(shell find ./tests -type f -name '*.h' -or -name '*.hpp' -or -name '*.c' -or -name '*.cpp' | sort)
CPP_FILES_TO_FORMAT=${TEST_FILES_TO_FORMAT}
